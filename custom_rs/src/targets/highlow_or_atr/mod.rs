...
use crate::targets::common::{
    ...
    barrier::compute_highlow_or_atr_targets_and_rr,
};

pub(crate) const ID: &str = "highlow_or_atr";
...
pub(crate) fn attach(frame: &mut TargetFrame<'_>, config: &Config) -> Result<()> {
    attach_highlow_with_atr(frame, config, ID, compute_highlow_or_atr_targets_and_rr)
}
