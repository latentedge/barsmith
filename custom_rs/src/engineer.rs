...
impl FeatureEngineer {
    ...
    fn attach_next_bar_targets(&mut self, config: &Config) -> Result<()> {
        ...
        rr_series.rename("rr_next_bar_color_and_wicks".into());
        self.replace_series(rr_series)?;

        let mut exit_series = self.frame.column(exit_source)?.clone();
        exit_series.rename("next_bar_color_and_wicks_exit_i".into());
    ...
    fn attach_wicks_kf_targets(&mut self, config: &Config) -> Result<()> {
        ...
        let mut rr_series = self.frame.column(rr_source)?.clone();
        rr_series.rename("rr_wicks_kf".into());
        self.replace_series(rr_series)?;

        let mut exit_series = self.frame.column(exit_source)?.clone();
        exit_series.rename("wicks_kf_exit_i".into());
        self.replace_series(exit_series)?;
        Ok(())
    }
    ...
    fn attach_highlow_or_atr_targets(&mut self, config: &Config) -> Result<()> {
        ...
        let mut rr_series = self.frame.column(rr_source)?.clone();
        rr_series.rename("rr_highlow_or_atr".into());
        self.replace_series(rr_series)?;

        let mut eligible_series = self.frame.column(eligible_source)?.clone();
        eligible_series.rename("highlow_or_atr_eligible".into());
        self.replace_series(eligible_series)?;

        let mut exit_series = self.frame.column(exit_source)?.clone();
        exit_series.rename("highlow_or_atr_exit_i".into());
        self.replace_series(exit_series)?;
        Ok(())
    }
    ...
    fn attach_highlow_1r_targets(&mut self, config: &Config) -> Result<()> {
        ...
        let mut rr_series = self.frame.column(rr_source)?.clone();
        rr_series.rename("rr_highlow_1r".into());
        self.replace_series(rr_series)?;

        let mut eligible_series = self.frame.column(eligible_source)?.clone();
        eligible_series.rename("highlow_1r_eligible".into());
        self.replace_series(eligible_series)?;

        let mut exit_series = self.frame.column(exit_source)?.clone();
        exit_series.rename("highlow_1r_exit_i".into());
        self.replace_series(exit_series)?;
        Ok(())
    }
    ...
    fn attach_2x_atr_tp_atr_stop_targets(&mut self, config: &Config) -> Result<()> {
        ...
        let mut rr_series = self.frame.column(rr_source)?.clone();
        rr_series.rename("rr_2x_atr_tp_atr_stop".into());
        self.replace_series(rr_series)?;

        let mut eligible_series = self.frame.column(eligible_source)?.clone();
        eligible_series.rename("2x_atr_tp_atr_stop_eligible".into());
        self.replace_series(eligible_series)?;
    ...
    fn attach_3x_atr_tp_atr_stop_targets(&mut self, config: &Config) -> Result<()> {
        ...
        let mut rr_series = self.frame.column(rr_source)?.clone();
        rr_series.rename("rr_3x_atr_tp_atr_stop".into());
        self.replace_series(rr_series)?;

        let mut eligible_series = self.frame.column(eligible_source)?.clone();
        eligible_series.rename("3x_atr_tp_atr_stop_eligible".into());
        self.replace_series(eligible_series)?;
    ...
    fn attach_atr_tp_atr_stop_targets(&mut self, config: &Config) -> Result<()> {
        ...
        let mut rr_series = self.frame.column(rr_source)?.clone();
        rr_series.rename("rr_atr_tp_atr_stop".into());
        self.replace_series(rr_series)?;

        let mut eligible_series = self.frame.column(eligible_source)?.clone();
        eligible_series.rename("atr_tp_atr_stop_eligible".into());
        self.replace_series(eligible_series)?;
    ...
    fn attach_highlow_sl_2x_atr_tp_rr_gt_1_targets(&mut self, config: &Config) -> Result<()> {
        ...
        let mut rr_series = self.frame.column(rr_source)?.clone();
        rr_series.rename("rr_highlow_sl_2x_atr_tp_rr_gt_1".into());
        self.replace_series(rr_series)?;

        let mut eligible_series = self.frame.column(eligible_source)?.clone();
        eligible_series.rename("highlow_sl_2x_atr_tp_rr_gt_1_eligible".into());
        self.replace_series(eligible_series)?;

        let mut exit_series = self.frame.column(exit_source)?.clone();
        exit_series.rename("highlow_sl_2x_atr_tp_rr_gt_1_exit_i".into());
        self.replace_series(exit_series)?;
        Ok(())
    }
    ...
    fn attach_highlow_sl_1x_atr_tp_rr_gt_1_targets(&mut self, config: &Config) -> Result<()> {
        ...
        let mut rr_series = self.frame.column(rr_source)?.clone();
        rr_series.rename("rr_highlow_sl_1x_atr_tp_rr_gt_1".into());
        self.replace_series(rr_series)?;

        let mut eligible_series = self.frame.column(eligible_source)?.clone();
        eligible_series.rename("highlow_sl_1x_atr_tp_rr_gt_1_eligible".into());
        self.replace_series(eligible_series)?;

        let mut exit_series = self.frame.column(exit_source)?.clone();
        exit_series.rename("highlow_sl_1x_atr_tp_rr_gt_1_exit_i".into());
        self.replace_series(exit_series)?;
        Ok(())
    }
    ...
    fn attach_highlow_or_atr_tightest_stop_targets(&mut self, config: &Config) -> Result<()> {
        ...
        let mut rr_series = self.frame.column(rr_source)?.clone();
        rr_series.rename("rr_highlow_or_atr_tightest_stop".into());
        self.replace_series(rr_series)?;

        let mut eligible_series = self.frame.column(eligible_source)?.clone();
        eligible_series.rename("highlow_or_atr_tightest_stop_eligible".into());
        self.replace_series(eligible_series)?;

        let mut exit_series = self.frame.column(exit_source)?.clone();
        exit_series.rename("highlow_or_atr_tightest_stop_exit_i".into());
        self.replace_series(exit_series)?;
        Ok(())
    }

    fn attach_tribar_4h_targets(&mut self, config: &Config) -> Result<()> {
        // The Tribar target is defined only for 4h bars; the filename is the
        // only reliable signal available before loading private datasets.
        ...
        rr_series.rename("rr_tribar_4h_2atr".into());
        self.replace_series(rr_series)?;
        ...
        exit_series.rename("tribar_4h_2atr_exit_i".into());
        self.replace_series(exit_series)?;
    ...
    fn replace_series<C>(&mut self, series: C) -> Result<()>
    where
        C: IntoColumn,
    {
...
fn volatility_features(
    ...
    // `atr` is exported for target generation but excluded from the core
    // NaN-trimming set so it does not change the warmup window.
