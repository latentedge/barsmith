...
fn catalog_order_without_feature_pairs_is_bool_then_fc() -> Result<()> {
    ...
    // Tiny fixtures may not produce scalar thresholds. In that case the
    // boolean portion is the only ordering worth checking.
...
fn catalog_order_with_feature_pairs_is_bool_then_fc_then_ff() -> Result<()> {
    ...
    // Tiny engineered datasets may have no scalar or pairwise predicates; the
    // boolean ordering check above is the meaningful assertion in that case.
