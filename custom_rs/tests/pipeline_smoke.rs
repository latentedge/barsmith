...
#[test]
fn pipeline_produces_results_and_metadata() {
    let temp_dir = tempdir().expect("tempdir");
    let csv_path = temp_dir.path().join("sample.csv");
    fs::write(&csv_path, SAMPLE_DATA).expect("write sample csv");
    ...
    assert!(
        output_dir.join("cumulative.duckdb").exists(),
        "DuckDB catalog missing"
    );
}
