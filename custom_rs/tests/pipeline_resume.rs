...
fn load_combinations_and_count(results_dir: &Path) -> Result<(usize, usize)> {
    ...
    let unique = combo_only.unique_stable(None, UniqueKeepStrategy::First, None)?;
...
fn feature_pairs_depth_two_then_one_preserves_results() -> Result<()> {
    ...
    // The lower-depth rerun should be a no-op because the first run already
    // covered the depth-1 window under the same catalog identity.
...
fn feature_pairs_toggle_after_zeros_causes_run_identity_mismatch() -> Result<()> {
    ...
    let err = match run_pipeline_for_test(cfg2) {
        Ok(_) => panic!("feature-pairs toggle should be rejected"),
        Err(error) => error,
    };
    assert!(
    ...
    // Results from the first run should be preserved after the rejected run.
    let (unique_after_2, rows_after_2) = load_combinations_and_count(&results_dir)?;
    assert_eq!(
        unique_after_2, unique_after_1,
        "rejected run should not add or remove stored combinations"
    );
    assert_eq!(
        rows_after_2, rows_after_1,
        "rejected run should not change stored rows"
    );

    Ok(())
}

/// Changing enable_feature_pairs changes the effective catalog and therefore
/// the run identity.
#[test]
fn feature_pairs_toggle_causes_run_identity_mismatch_error() -> Result<()> {
    ...
    assert_eq!(
        unique_after_2, unique_after_1,
        "rejected feature_pairs run should not change stored combinations"
    );
    assert_eq!(
        rows_after_2, rows_after_1,
        "rejected feature_pairs run should not change stored rows"
    );
...
/// Tests that toggling enable_feature_pairs after a run is rejected by the
...
fn pipeline_resume_feature_pairs_toggle_causes_run_identity_mismatch() -> Result<()> {
    ...
    // produces a different catalog and should fail for the existing output
    // directory.
    let mut config3 = base_config(&csv_path, &output_dir);
    config3.enable_feature_pairs = true;
    let err = match run_pipeline_for_test(config3) {
        Ok(_) => panic!("feature-pairs toggle should be rejected"),
        Err(error) => error,
    };
    assert!(
    ...
    // Results from the previous run should be preserved after the rejected
    // feature-pair toggle.
