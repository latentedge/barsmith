...
// Older examples use this target name; it maps to the built-in upward target.
...
fn read_base_ohlcv(path: &Path) -> Result<DataFrame> {
    ...
    let df = CsvReadOptions::default()
        ...
        .finish()
        .with_context(|| format!("Failed to collect CSV data from {display_path}"))?;
    Ok(df)
}

fn series_to_f64(series: &Column) -> Result<Vec<f64>> {
    match series.dtype() {
...
pub fn prepare_dataset_with_options(
    ...
    df.with_column(Series::new("is_green".into(), is_green).into())?;
    df.with_column(Series::new("is_red".into(), is_red).into())?;
    df.with_column(Series::new("is_inside_bar".into(), is_inside_bar).into())?;
    df.with_column(Series::new("is_range_expansion".into(), is_range_expansion).into())?;
    df.with_column(Series::new("range".into(), range).into())?;
    df.with_column(Series::new("body".into(), body).into())?;
    df.with_column(Series::new("abs_body".into(), abs_body).into())?;
    df.with_column(Series::new("sma_9".into(), sma_9).into())?;
    df.with_column(Series::new("sma_20".into(), sma_20).into())?;
    df.with_column(Series::new("sma_50".into(), sma_50).into())?;

    df.with_column(Series::new(TARGET_NEXT_BAR_UP.into(), next_bar_up).into())?;
    df.with_column(Series::new(TARGET_NEXT_BAR_DOWN.into(), next_bar_down).into())?;
    // Compatibility alias (same semantics as next_bar_up in the builtin engine).
    let mut target_alias = df.column(TARGET_NEXT_BAR_UP)?.clone();
    target_alias.rename(TARGET_NEXT_BAR_COLOR_AND_WICKS.into());
    df.with_column(target_alias)?;
    ...
    df.with_column(Series::new(eligible_up.as_str().into(), eligible.clone()).into())?;
    df.with_column(Series::new(eligible_down.as_str().into(), eligible.clone()).into())?;
    df.with_column(Series::new(eligible_alias.as_str().into(), eligible.clone()).into())?;
    ...
    df.with_column(Series::new(exit_up.as_str().into(), exit_i.clone()).into())?;
    df.with_column(Series::new(exit_down.as_str().into(), exit_i.clone()).into())?;
    df.with_column(Series::new(exit_alias.as_str().into(), exit_i.clone()).into())?;
    ...
    df.with_column(Series::new(rr_up.as_str().into(), rr_next_bar_up).into())?;
    df.with_column(Series::new(rr_down.as_str().into(), rr_next_bar_down).into())?;
    let mut rr_alias_column = df.column(&rr_up)?.clone();
    rr_alias_column.rename(rr_alias.as_str().into());
    df.with_column(rr_alias_column)?;
