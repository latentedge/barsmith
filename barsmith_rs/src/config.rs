...
pub enum StackingMode {
    /// Treat every mask-hit bar as an independent trade sample.
    Stacking,
...
pub enum PositionSizingMode {
    /// Treat 1R as `risk_pct_per_trade` of current equity and apply RR directly.
    Fractional,
...
pub struct Config {
    ...
    /// even if that value is zero. This distinguishes "no override" from an
    /// explicit `resume-from=0` when applying stored DuckDB resume metadata.
    #[serde(default)]
    ...
    /// Prune supersets of combinations whose sample count is non-zero but
    /// below the storage floor.
    #[serde(default)]
    pub strict_min_pruning: bool,
    /// Cache zero-sample depth-2 pairs and skip higher-depth combinations
    /// that contain them.
    #[serde(default)]
    ...
    /// Optional cap on feature-to-feature comparison predicates.
    #[serde(default)]
    ...
    ///
    /// `Core` keeps the ranking/storage metrics cheap; `Full` adds the broader
    /// shape metrics used in detailed reports.
    #[serde(default)]
    ...
    /// Optional engineered-dataset horizon in years, inferred from timestamps
    /// and used to annualize equity metrics.
    #[serde(default)]
    ...
    /// Optional eval-time gate. When non-empty, Barsmith evaluates only
    /// combinations that include at least one of these post-pruning feature names.
    #[serde(default)]
