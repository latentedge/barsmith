...
use barsmith_rs::{
    Config, Direction, ReportMetricsMode,
...
#[test]
fn resume_rejects_catalog_hash_changes_in_existing_output_dir() -> Result<()> {
    ...
    // must fail loudly. The feature catalog changes the enumeration universe,
    // so reusing the previous resume offset or existing combination cache
    // would be unsafe.
    let mut cfg_b = base_config(&csv_path, &output_dir);
    cfg_b.catalog_hash = Some("B".to_string());
    let err = match CumulativeStore::new(&cfg_b) {
        Ok(_) => panic!("catalog hash change should be rejected"),
        Err(error) => error,
    };
    assert!(
        err.to_string().contains("different Barsmith run identity"),
...
/// Changing runtime knobs like `batch_size` should not affect the run identity
/// used for resume metadata. This test verifies that a second run with a
/// different batch size still picks up the previously processed index rather
...
/// Toggling auto_batch should not affect the run identity or resume offset;
/// it only changes how batch sizes are tuned within a run.
#[test]
...
#[test]
fn resume_allows_max_depth_extension_in_existing_output_dir() -> Result<()> {
    let temp_dir = tempdir()?;
    let csv_path = temp_dir.path().join("data.csv");
    let output_dir = temp_dir.path().join("out_depth_change");
    fs::write(&csv_path, "timestamp,open\n2024-01-01T00:00:00Z,1\n")?;
    ...
    assert_eq!(
        resume2,
        combos.len() as u64,
        "increasing max_depth should continue after the already processed prefix"
    );
...
#[test]
fn resume_rejects_existing_state_without_manifest() -> Result<()> {
    let temp_dir = tempdir()?;
    let csv_path = temp_dir.path().join("data.csv");
    let output_dir = temp_dir.path().join("out_missing_manifest");
    let results_dir = output_dir.join("results_parquet");
    fs::create_dir_all(&results_dir)?;
    fs::write(&csv_path, "timestamp,open\n2024-01-01T00:00:00Z,1\n")?;
    fs::write(results_dir.join("part-0000000000000000.parquet"), b"legacy")?;

    let cfg = base_config(&csv_path, &output_dir);
    let err = match CumulativeStore::new(&cfg) {
        Ok(_) => panic!("state without manifest should be rejected"),
        Err(error) => error,
    };
    assert!(
        err.to_string().contains("run_manifest.json is missing"),
        "unexpected error: {err:?}"
    );
